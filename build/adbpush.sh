...
if [[ "$adb_state" == *"command not found"* ]]; then #adb binary not found
  echo $adb_state
  echo "adb binary not found"
elif [[ "$adb_state" == *"error"* ]]; then #adb not connected to device
  echo $adb_state
elif [[ "$source_path" == *"No such file"* ]]; then #no build found
  echo $source_path
else
