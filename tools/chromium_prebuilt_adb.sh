...
echo "WARNING: THIS ISN'T A RECOMMENDED WAY OF GENERATING PREBUILT CHROMIUM."
echo "ONLY USE THIS SCRIPT IF YOU HAVE THE LATEST VERSION OF THIS ROM,"
echo "OR A SIMILAR ROM USING SAME CHROMIUM."
echo ""

command -v adbd >/dev/null && ADB=1 || ADB=0
...
touch device.txt
echo $CHECK_DEVICE > device.txt
NUMBER=$(grep -roh device device.txt | wc -w)
if [[ $NUMBER == 1 ]]; then
    echo "No android device attach. Make sure USB debugging is on."
    rm -rf device.txt
    return 1
elif [[ $NUMBER > 2 ]]; then
    echo "More than one android device attatched."
    rm -rf device.txt
    return 1
fi
rm -rf device.txt
...
echo "Copying files..."
adb pull /system/app/webview $PREBUILT_DIR/app
adb pull /system/lib/libwebviewchromium.so $PREBUILT_DIR/lib/libwebviewchromium.so
adb pull /system/lib/libwebviewchromium_plat_support.so $PREBUILT_DIR/lib/libwebviewchromium_plat_support.so
adb pull /system/lib/libwebviewchromium_loader.so $PREBUILT_DIR/lib/libwebviewchromium_loader.so
if [[ $ARCH == arm64 ]]; then
    mkdir -p $PREBUILT_DIR/lib64
    adb pull /system/lib64/libwebviewchromium.so $PREBUILT_DIR/lib64/libwebviewchromium.so
    adb pull /system/lib64/libwebviewchromium_plat_support.so $PREBUILT_DIR/lib64/libwebviewchromium_plat_support.so
    adb pull /system/lib64/libwebviewchromium_loader.so $PREBUILT_DIR/lib64/libwebviewchromium_loader.so
fi
